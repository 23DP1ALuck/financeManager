...
const Header = ({sections} : HeaderProps) => {
    ...
    return (
        <div className="flex justify-between w-full sm:px-7 px-2 py-1 bg-gray-100">
            <div className="flex">
                <div className="flex items-center py-2.5 gap-2 md:px-6 lg:gap-6 w-full">
                    ...
                    <div className="flex align-center justify-between w-full">
                        <div className="hidden sm:flex justify-center items-center px-2.5 gap-2.5">
                            {sections.map(({section, sectionName, href}) => (
                                <Link
                                    className={`${pathname === href ? "bg-[#7EF9D28b] font-bold" : "bg-[#00000005] font-semibold duration-200 hover:bg-[#0000002b]"} lg:p-2.5 p-1.5 text-sm rounded-3xl h-fit"`}
