...
const TransactionInfo = ({transaction} : TransactionInfoProps) => {
    ...
    return(
        <div className="flex flex-col w-1/2 bg-white px-5 py-7 rounded-sm">
            {transactionsExists ? <div className="flex flex-col gap-1">
                <h1 className="font-medium text-base text-black/90">Name: <span>{transaction?.transaction_name}</span></h1>
                <h1 className="font-medium text-base text-black/90">Description: <span>{transaction?.transaction_description}</span></h1>
                <h1 className="font-medium text-base text-black/90">Category: <span>{transaction?.category.name}</span></h1>
                <h1 className="font-medium text-base text-black/90">Date: <span>{moment(transaction?.date).format("MMM DD YYYY")}</span></h1>
                <h1 className="font-medium text-base text-black/90">Wallet: <span>{transaction?.account ? transaction.account.name : "Deleted wallet"}</span></h1>
